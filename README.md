# STrUDeL: **S**treamline **Tr**actography **U**tilities for **De**ep **L**earning
...
```
3. Compute a tissue-type mask with MRTrix and FSL: `T1_5tt.nii.gz`
```
...
1. Compute the average b0 from the diffusion data: `dmri_b0.nii.gz`
```
dwiextract dmri.nii.gz -fslgrad dmri.bvec dmri.bval - -bzero | mrmath - mean dmri_b0.nii.gz -axis 3
```
2. Compute a rigid transform between the average b0 and T1: `dmri2T1_0GenericAffine.mat`
```
...
```
3. Move the structural masks to diffusion space: `dmri_mask.nii.gz`, `dmri_seed.nii.gz`, `dmri_5tt.nii.gz`
```
...
```
4. Fit the diffusion data with FODs: `dmri_tournier.txt`, `dmri_fod.nii.gz`
```
...
```
5. Perform SDStream tractography: `dmri_sdstream.tck`
```
...
```
2. Apply that transformation to the T1 and masks at 2mm isotropic resolution: `T1_N4_mni_2mm.nii.gz`, `T1_mask_mni_2mm.nii.gz`, `T1_5tt_mni_2mm.nii.gz`, `T1_seed_mni_2mm.nii.gz`
```
...
```
3. Apply that transformation to the FODs with reorientation, first walking through T1 space: `T1_fod_mni_2mm.nii.gz`
```
...
```
4. Apply that transformation to the tractogram, first walking through T1 space: `T1_sdstream.tck`, `T1_sdstream_mni_2mm.tck`
```
...
* For each streamline, we compute the ground truth label, `∆x`, for each point using `utils.vox2step()`. `step` is the prefix.
* For each streamline, we convert each point to 11-dimensional trilinear interpolation format, `c`, using `utils.vox2trid()` for the first three elements and `utils.vox2trii()` for the last 8. `trid` and `trii` are the prefixes, respectively.
...
These outputs are then saved in a list for all streamlines in the batch and subsequently saved as packed tensors (`trii` and `trid`) or padded tensors (`step`) along with a padded tensor allowing the padding to be ignored during loss computation, `mask`, computed with `utils.len2mask()`. These data for each batch are then saved in the `pt` folder as `<prefix>_<batch_number>.pt`.
...
First, write the `pt` folder into a text file so the DataLoader knows where to look during training for the prepared labels and streamline batches. Note the above preparation steps can be repeated for other data to enable training on multiple datasets.
```
echo ${pwd}/pt > ../data.txt
```
Then, train the model with the data recorded in that text file for 100 epochs, randomly selecting a batch for each subject (in this case only 1) for each epoch. This script uses functions defined in `data.py`, `modules.py`, and `utils.py`. These functions contain the majority of the logic outlined in the [STrUDeL paper](#authors-and-reference).
```
cd ..
